...
// npx jest tests/post/sub-test/like/comment/like-comment.spec.ts

describe('POST /api/posts/current/comment/:commentId/likes', () => {
  ...
  it('Must successfully like the comment post', async () => {
  ...
  it('Must fail like the comment post if commentId is invalid', async () => {
    const loginRes = await supertest(app.getHttpServer())
      .post('/api/users/login')
      .send({
        userName: 'USERTEST123',
    ...
    const result = await supertest(app.getHttpServer())
      .post(`/api/posts/current/comments/INVALID COMMENT ID/likes`)
      .set('Cookie', cookieAccessToken);
