...
// npx jest tests/post/sub-test/like/sub-comment/unlike-sub-comment.spec.ts

describe('DELETE /api/posts/current/comment/current/sub-comments/:subCommentId/likes', () => {
  ...
  it('Must successfully unlike the sub comment', async () => {
  ...
  it('Must fail unlike the sub comment if subCommentId is invalid', async () => {
    const loginRes = await supertest(app.getHttpServer())
      .post('/api/users/login')
      .send({
        userName: 'USERTEST123',
